...
async def retrain_fraud_model(
    ...
    return {
        ...
        "message": "Fraud model retraining started in background."
    }
...
async def promote_fraud_model(
    ...
    try:
        ...
        if threshold_value is None:
            raise HTTPException(status_code=400, detail="Missing best_threshold")
...
async def get_fraud_runs(limit: int = 5):
    ...
    runs_df = runs_df[columns].copy()

    runs_df["start_time"] = runs_df["start_time"].astype(str)

    runs_df = runs_df.replace({pd.NA: None})
